import {
  ...
  Renderer2, effect, inject, input,
} from '@angular/core';

export type NgAtomsButtonVariant = 'primary' | 'outline' | 'ghost' | 'secondary' | 'destructive';
...
export class NgAtomsButtonDirective implements OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      if (this.loading()) { this._activateLoading(); }
      else { this._deactivateLoading(); }
    });
  }
  ...
  private _deactivateLoading(): void {
    ...
    if (!el.hasAttribute('disabled')) this.renderer.removeAttribute(el, 'aria-disabled');
  }
}
